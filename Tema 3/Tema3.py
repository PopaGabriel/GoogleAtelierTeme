...
def get_year_data(data: pd.DataFrame, year: str) -> dict:
    try:
        return {year: [(name, value) for name, value in data.loc[description[1].index(year)].iteritems()]}
    except ValueError:
        return dict(pd.DataFrame())


def get_country_data(data: pd.DataFrame, country: str):
    dictionary = collections.defaultdict(list)
    try:
        for i, val in enumerate(description[1]):
            dictionary[country].append((val, data[country][i]))
        return dictionary
    except KeyError:
        return dictionary


def perform_average(data: pd.Series) -> float:
    try:
        return data.mean()
    except KeyError:
        return None
...
if __name__ == '__main__':
    ...
    print(get_year_data(df, '2008'))
    print(get_year_data(df, '2011'))

    print(get_country_data(df, 'YY'))
    print(get_country_data(df, 'RO'))

    try:
        print(perform_average(df['RO']))
        print(perform_average(df['YY']))
    except KeyError:
        print('Nu este ce vreeeeau')
